...
from rsv_flux import flux_log, flux_lines
import concurrent.futures


def usage():
    print("Usage: python rsv_flux_daily.py <output_dir> <temp_dir> <log_files>\n")
    print("This script will parse the log files and create for each of them a ")
    print("flux_metric file in the temporary folder.\n")
...
class file_bucket:
    ...
    def load(self):
        with open(self.output_file, "w") as recap_file:
            rcf = flux_log(7200, 0, self.ip2a4, self.ip2a6, self.as_names, recap_file)
            nb_events = rcf.load_flux_log(self.source_file, time_start=self.time_start)
...
if __name__ == "__main__":
    ...
    for source_file in source_files:
        ...
        if item.startswith("queries"):
            item = item[7:]
        output_file = os.path.join(temp_dir, "flux-daily" + item + ".csv")
    ...
    for bucket in bucket_list:
        rcf.load_flux(bucket.output_file)

    nb_files = 0
    for key in rcf.cc_as_list:
        if rcf.cc_as_list[key].total_uids > 10000:
            as_file = os.path.join(output_dir, "flux-" + 
                                   rcf.cc_as_list[key].query_cc + "-" + 
                                   rcf.cc_as_list[key].query_AS + ".csv")
            rcf.cc_as_list[key].save_file(as_file)
            print("Saved: " + str(len(rcf.cc_as_list[key].slices)) + " slice in " + as_file)
            nb_files += 1
    print("Saved " + str(nb_files) + " CC/AS files.")

    summary_file = os.path.join(output_dir, "flux-summary.csv")
    df = rcf.summary_df()
