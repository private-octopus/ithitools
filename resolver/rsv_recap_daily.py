...
def usage():
    print("Usage: python rsv_recap_daily.py <output_dir> <temp_dir> <log_files>\n")
    print("This script will parse the log files and create for each of them a ")
    print("recap_metric file in the temporary folder.\n")
...
class file_bucket:
    ...
    def load(self):
        with open(self.output_file, "w") as recap_file:
            rcl = recap_log(7200, 0, self.ip2a4, self.ip2a6, self.as_names, recap_file)
            nb_events = rcl.load_recap_log(self.source_file, time_start=self.time_start)
